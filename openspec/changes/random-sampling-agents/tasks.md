...
- [ ] 4.3 Replace the hard-coded `random_move` calls in the play driver game loop with per-player `select_move(..., strategy_p0)` / `select_move(..., strategy_p1)` using the corresponding RNG instance
- [ ] 4.4 Add `"agent_p0"`, `"agent_p1"`, `"seed_p0"`, `"seed_p1"` fields to the `game_start` NDJSON log event
- [ ] 4.5 Update the `usage()` message to document the new `--agent1`, `--agent2`, `--seed1`, `--seed2` flags
- [ ] 4.6 Validate that an invalid `--agent1` or `--agent2` value prints an error to stderr and exits non-zero
...
- [ ] 6.4 Add `Advance`-weight test: verify approximately 50/50 split when one BuyPatch (weight 1.0) and one Advance are the only legal moves
...
- [ ] 7.2 Update `openspec/specs/engine/spec.md` by merging the `engine` delta spec (`--agent1`/`--agent2`/`--seed1`/`--seed2`, per-player NDJSON fields)
