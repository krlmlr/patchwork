...
- [ ] 1.2 Add `void GameSetup::to_ndjson(std::ostream& out) const` that writes one JSON line: `{"type":"setup","circle":"<33-char-string>"}` (converting IDs back to single-char names for output)
- [ ] 1.3 Add `GameSetup` to the umbrella header `src/patchwork.hpp`
...
- [ ] 2.2 Write all results to `src/generated/game_setups.hpp` as `constexpr std::array<std::string_view, kNumGameSetups> kGameSetups` with `inline constexpr std::size_t kNumGameSetups = 100`; no seeds or struct definition in the header
- [ ] 2.3 Ensure that re-running the script with a larger `N_SETUPS` produces a longer array whose first 100 entries are bit-for-bit identical to the previous output
...
- [ ] 4.1 Create `tests/test_game_setup.cpp` with a Catch2 test section covering construction from string:
...
- [ ] 4.3 Add a Catch2 test covering `to_ndjson`: construct a `GameSetup` from `kGameSetups[0]`, verify the output is valid single-line JSON containing `"type":"setup"` and the circle as a 33-char string
