...
static std::string hline_t(const char* left, const char* fill, const char* junction,
                           const char* right, int inner, int junction_pos) {
    std::string out = left;
...
static std::string hline_3t(const char* left, const char* fill, const char* jA, const char* jB,
                            const char* jC, const char* right, int inner, int posA, int posB,
                            int posC) {
    std::string out = left;
    for (int i = 0; i < inner; ++i) {
        if (i == posA)
...
static std::string ndjson_sep_line(int iw, std::initializer_list<int> junctions, int height) {
...
static void print_row(const std::string& content, int visible_len, int iw) {
    ...
    std::printf("%s %s%s %s\n", kBdr_V, content.c_str(),
                std::string(static_cast<std::size_t>(pad), ' ').c_str(), kBdr_V);
}

/// Render patch-circle sequence: 33 chars.
static std::string circle_line(const SimplifiedGameState& state, const GameSetup& setup) {
    std::string out;
    for (int i = 0; i < 33; ++i) {
...
static std::vector<int> collect_buyable(const SimplifiedGameState& state, const GameSetup& setup,
                                        int max_detail) {
    std::vector<int> ids;
...
static void render_narrow(const SimplifiedGameState& state, const GameSetup& setup,
    ...
    // ── Top border.
    {
        ...
        if (mid < 0) mid = 0;
        std::printf("%s%s%s%s%s\n", kBdr_TL, left_txt.c_str(), rep(kBdr_H, mid).c_str(),
    ...
    static const char* const kKeys[] = {"[1/2/3]buy  [a]adv      [q]quit        ",
    ...
    for (int i = 0; i < detail_count; ++i) {
        ...
        if (i < static_cast<int>(buyable.size())) {
            ...
            char buf[60];
            std::snprintf(buf, sizeof(buf), "[%2d] %c  cost %2d  time %2d  inc %d", i + 1, p.name,
                          p.buttons, p.time, p.income);
            if (C) {
        ...
        std::printf("%s%s %s\n", std::string(static_cast<std::size_t>(pad), ' ').c_str(), key_hint,
                    kBdr_V);
    }
    ...
    std::snprintf(stat0, sizeof(stat0), " P1  btn %3d  inc %2d  pos %2d  fr %2d ",
                  state.player(0).buttons(), state.player(0).income(), state.player(0).position(),
                  state.player(0).free_spaces());
    std::snprintf(stat1, sizeof(stat1), " P2  btn %3d  inc %2d  pos %2d  fr %2d  ",
                  state.player(1).buttons(), state.player(1).income(), state.player(1).position(),
    ...
    std::string p0col =
        C ? (std::string(kColorP1) + (active == 0 ? kBold : "") + pad_to(stat0, mid_col) + kReset)
    ...
    std::printf("%s\n", hline_3t(kBdr_LT, kBdr_H, kBdr_TT, kBdr_TT, kBdr_BT, kBdr_RT, IW, pos_q1,
                                 pos_q2, mid_col)
                            .c_str());
    ...
    {
        char hdr[200];
        std::snprintf(hdr, sizeof(hdr), " P1 quilt  %s P2 quilt  %s %-*s", kBdr_V, kBdr_V,
                      log_pane_width - 1, "Event log");
        std::printf("%s%s%s\n", kBdr_V, hdr, kBdr_V);
    }
    ...
    for (int row = 0; row < 9; ++row) {
        ...
        if (row < log_visible) {
            ...
            if (log.wrap_mode) {
                ...
                if (off < static_cast<int>(e.size()))
                    display =
                        e.substr(static_cast<std::size_t>(off), static_cast<std::size_t>(usable));
            }
        ...
        std::printf("%s ????????? %s ????????? %s %s%s %s\n", kBdr_V, kBdr_V, kBdr_V,
                    log_cell.c_str(), std::string(static_cast<std::size_t>(pad), ' ').c_str(),
                    kBdr_V);
    }
...
static void render_wide(const SimplifiedGameState& state, const GameSetup& setup,
                        const LogState& log, const NdjsonState& ndjson, const DisplayConfig& cfg) {
    const int W = cfg.width;
    const int IW = W - 2;
    const bool C = cfg.color_enabled;
    const int active = state.active_player();
    ...
    // ── Top border: ┌─left─┬─Q1─┬─Q2─┬─event─▶P1─┐
    {
        ...
        std::printf("%s%s%s%s%s%s%s%s%s%s%s\n", kBdr_TL, left_txt.c_str(),
                    rep(kBdr_H, left_fill).c_str(), kBdr_TT, rep(kBdr_H, Q_INNER).c_str(), kBdr_TT,
                    rep(kBdr_H, Q_INNER).c_str(), kBdr_TT, rep(kBdr_H, event_fill).c_str(),
                    right_txt.c_str(), kBdr_TR);
    }
    ...
    static const char* const kWideKeys[] = {"[1/2/3]buy  [a]adv      [q]quit        ",
                                            "[z/u]undo [Z/r]redo   [</>]log  [w]wrap",
                                            "[m]v [f]^ [h]^/2  [,]- [.]+            "};
    ...
    for (int i = 0; i < detail_count && (2 + i) < TOTAL - 2; ++i) {
        ...
        std::string plain_row =
            plain_buf + std::string(static_cast<std::size_t>(pad), ' ') + key_hint;
        ...
        if (C && i < static_cast<int>(buyable.size())) {
            ...
            lcolored[2 + i] = col + plain_buf + kReset +
                              std::string(static_cast<std::size_t>(pad), ' ') + key_hint;
        } else {
    ...
    std::string p0col_colored =
        C ? (std::string(kColorP1) + (active == 0 ? kBold : "") + p0col_str + kReset) : p0col_str;
    std::string p1col_colored =
        C ? (std::string(kColorP2) + (active == 1 ? kBold : "") + p1col_str + kReset) : p1col_str;
    ...
    for (int row = 0; row < TOTAL; ++row) {
        ...
        if (row == TOTAL - 2) {
            ...
            std::string sep = kBdr_LT + rep(kBdr_H, stats_mid) + kBdr_TT +
                              rep(kBdr_H, LEFT - stats_mid - 1) + kBdr_RT;
            std::printf("%s%s%s%s%s%s%s\n", sep.c_str(), q1.c_str(), kBdr_V, q2.c_str(), kBdr_V,
                        ev.c_str(), kBdr_V);
        } else if (row == TOTAL - 1) {
            // Stats row: │ P1 stats │ P2 stats │ Q1 │ Q2 │ event │
            std::printf("%s%s%s%s%s%s%s%s%s%s%s\n", kBdr_V, p0col_colored.c_str(), kBdr_V,
                        p1col_colored.c_str(), kBdr_V, q1.c_str(), kBdr_V, q2.c_str(), kBdr_V,
                        ev.c_str(), kBdr_V);
        } else {
            ...
            if (lpad < 0) lpad = 0;
            std::printf("%s %s%s %s%s%s%s%s%s%s\n", kBdr_V, colored.c_str(),
                        std::string(static_cast<std::size_t>(lpad), ' ').c_str(), kBdr_V,
                        q1.c_str(), kBdr_V, q2.c_str(), kBdr_V, ev.c_str(), kBdr_V);
        }
    }

    // ── NDJSON separator: ├──stats_mid──┴──left_rest──┴──Q1──┴──Q2──┴─label─┤
    std::printf("%s\n",
                ndjson_sep_line(IW, {stats_mid, pos_lq, pos_q1q2, pos_q2ev}, eff_ndjson).c_str());
...
DisplayConfig init_display(bool no_color_flag, int /*argc*/, char** /*argv*/) {
    ...
    if (cfg.width < 80 || cfg.height < 24) {
        std::fprintf(stderr, "Terminal too small: %dx%d. Minimum required: 80x24.\n", cfg.width,
                     cfg.height);
        std::exit(1);
    }
    if (no_color_flag) {
...
void append_log(LogState& log, std::string entry) {
    ...
    if (static_cast<int>(log.entries.size()) > 50) log.entries.erase(log.entries.begin());
    log.scroll_offset = 0;
}
...
void ndjson_toggle_minimize(NdjsonState& s, int /*max_lines*/) {
    if (s.height > 0) {
        ...
        s.height = s.prev_height > 0 ? s.prev_height : 5;
}
